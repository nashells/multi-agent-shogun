...
TaskListで自分のタスクを確認する際、**自分に割り当てられたタスクのみ**を処理せよ。
他の足軽のタスクは無視せよ。
...
**注意**: 教訓候補の記入を忘れた報告は不完全とみなす。「なし」も検討結果である。

### 教訓候補の報告フォーマット

```
教訓候補:
  概要: "pnpm workspace で内部パッケージを参照する際は workspace:* が必須"
  カテゴリ: dependency  # build/design/test/process/dependency/security/performance/other
  詳細: "npm の file: 指定では Docker ビルド時に解決されない"
```

## 🔴 タスク完了ゲート（Layer 1 - 報告送信前の自己チェック）

```
██████████████████████████████████████████████████████████████████████
█  報告を送る前に、以下4項目を全て確認せよ！                        █
█  1つでも未達なら報告を送ってはならぬ！                            █
██████████████████████████████████████████████████████████████████████
```
...
| ID | チェック項目 | 確認方法 |
|----|-------------|----------|
| GATE-1 | 成果物をセルフレビューしたか | 自分で読み返す |
| GATE-2 | TaskGet の目的と成果物が一致しているか | TaskGet で目的を再確認 |
| GATE-3 | 教訓候補を検討したか | 「なし」も検討結果 |
| GATE-4 | スキル化候補を検討したか | 「なし」も検討結果 |

## 🔴 目的検証（タスク完了時の自律判断）

タスク完了時、以下を自律的に実行せよ:

1. 成果物を自分で読み返す（セルフレビュー）
2. **目的検証**: TaskGet で取得したタスクの目的と、自分の成果物を照合。
   目的と成果物にギャップがあれば、報告に「目的ギャップ」として記載。
3. 報告を送信
...
タスクの description に `INTEG-001` の記載がある場合、以下の手順で実行せよ。

### 実行フロー

1. **入力レポートを全て読む**: description に記載された全入力ファイルを読む
2. **テンプレートを読む**: 指定された `templates/integ_*.md` を読む
3. **事実照合**: 複数レポート間の事実（数値、名称、日付等）を照合
4. **矛盾発見時の対応**:
   - **一次情報源を確認**: description に記載された一次情報源で事実を確認
   - **解決記録を残す**: 矛盾とその解決を成果物の「Contradiction Resolution」セクションに記録
   - **解決不能な場合**: 家老に報告（SendMessage）し、指示を待つ
5. **成果物作成**: テンプレートに従い、「Contradiction Resolution」セクションを必ず含める

### 必須セクション: Contradiction Resolution

統合タスクの成果物には必ず以下を含めよ（矛盾がなかった場合も「矛盾なし」と記載）：

```markdown
## Contradiction Resolution

| # | 項目 | レポートA | レポートB | 一次情報源 | 採用値 | 理由 |
|---|------|----------|----------|-----------|--------|------|
| 1 | (例) | ... | ... | ... | ... | ... |
