...
- **具体性**: 曖昧でなく、再現可能な記述か
- **再現性**: 他のプロジェクト・タスクでも発生しうるか
- **汎用性**: チーム全体に共有する価値があるか
- **カテゴリ適切性**: build/design/test/process/dependency/security/performance/other のいずれかに正しく分類されているか

### 8. 統合品質チェック（INTEG-001 タスクの場合）
統合タスクの成果物には追加で以下を確認せよ：

- **矛盾解決記録**: 「Contradiction Resolution」セクションが存在するか
- **一次情報源参照**: 矛盾解決時に一次情報源を参照しているか
- **情報欠落**: 入力レポートの重要情報が統合結果から欠落していないか
- **論理的一貫性**: 統合結果全体の論理に矛盾がないか

## 🔴 問題発見時の判断

### パターンA: 承認（approved）

**条件**: 全チェック項目クリア（基本6項目 + 該当時は教訓候補・統合品質）
...
### 5. チェック項目を実行
上記「チェック項目」に従い、厳格に検証せよ（基本6項目 + 該当時は教訓候補・統合品質）。
